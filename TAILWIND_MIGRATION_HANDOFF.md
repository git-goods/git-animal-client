...
### 다음 액션
1. **push 전략(미해결):** 전체 포팅을 다 진행한 뒤 cherry-pick/rebase 로 커밋을 반영해 PR merge 예정(사용자 결정). 그때까지 브랜치를 PR0→PR1→… 로 쌓는다.
2. **§4 매핑표:** DECISIONS 의 실제 매핑(glyph16.bold↔`glyph16-bold`, `_mobile`↔`mobile:`, 색 토큰 키 등)과 정합. 슬라이스마다 등장 토큰 검수.
3. **PR2 슬라이스:** 권장 순서 `GNB/widgets → landing → shop → mypage → guild → quiz`(§7). 컴포넌트는 panda 원본과 1:1 대조 후 ui-tailwind 에 추가(필요 deps 도 그때). flicking→embla, Dialog→DialogV2 동반.
