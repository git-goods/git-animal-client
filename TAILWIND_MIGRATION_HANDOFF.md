...
### 다음 액션 (PR 1)
1. PR0 push & 리뷰/머지 여부 결정(아직 미push). 머지 후 `main` 기준 PR1 브랜치.
2. **§6-1 미해결**: 아직 `.style.ts` tailwind 파일이 없어 미검증. 슬라이스 전환 PR에서 §5 방침(인라인 className/cva) 준수하며 확인.
3. §4 토큰 매핑표를 코드 기준 최종 확정(슬라이스 변환 직전). DECISIONS의 매핑(glyph16.bold↔glyph16-bold, `_mobile`↔`mobile:` 등)과 정합.
4. PR1 슬라이스 착수 — 권장 순서 `auth → GNB/widgets → landing → ...`(§7). 컴포넌트는 panda 원본과 1:1 대조 후 ui-tailwind에 추가(필요 deps도 그때).
