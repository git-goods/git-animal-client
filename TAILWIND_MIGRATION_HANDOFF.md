...
- **검증:** type-check·`build:web` 통과, 잔존(_panda/styled-system) 0, 빌드 CSS 실측 일치(checkbox 4px/#fafafa/#a1a1aa, scrollArea thumb #e4e4e7, Label 14px, glyph 완전체, `[&_.heading]` arbitrary variant). **브라우저 시각 검증 완료**(프로덕션 `next start`).
- **ADR-014(테두리 함정):** 검증 중 ui-tailwind 컴포넌트 테두리가 안 보이는 버그 발견·수정. 앱 전역 `button{border:transparent}`(border-style:none)가 Tailwind `border`(width만)의 style 의존(저특이도 reset)을 덮음 → `Checkbox`/`Button`/`TextField` 에 `border-solid` 명시(특이도로 극복). 기존 Button/TextField 잠재 버그도 동시 해결. ⚠️ **turbopack `next dev --turbo` 는 새 패키지 컴포넌트 클래스 스캔이 불안정** → dev 검증은 `next build && next start` 또는 webpack dev 권장.

### ✅ PR 6 (guild)
- **설계(컴포넌트 5종):** ui-tailwind 에 `TextArea`/`SearchBar`(리프) + `Tooltip`/`DropdownMenu`/`CombineChip`(radix compound) 추가. compound 는 **visible 스타일만 GitAnimals override 로 1:1**, 애니메이션은 **표준 shadcn `tailwindcss-animate`**(fade/zoom/slide)로 대체(ADR-015, 사용자 방침). shadcn semantic 토큰은 미도입. `@radix-ui` select/tooltip/dropdown-menu + `tailwindcss-animate` deps·plugin 추가.
- **사용부:** guild 30파일 전환(병렬 에이전트 3개). 신규 컴포넌트 API 가 panda 와 동일해 import 소스만 교체. **ui-tailwind Button 은 panda 스타일 prop(mx/w/minWidth/px) 미지원 → className arbitrary 로 교정**(에이전트 누락분 type-check 로 잡아 수정). Dialog/CommonDialog/`dialogTitleStyle` panda 유지(ADR-012). 슬라이더(MemeberSlider/GuildPeopleList)는 이미 embla — css 만 전환. border 는 `border-solid` 동반(ADR-014).
- **검증:** type-check·`build:web` 통과, 잔존(_panda/styled-system) 0, 빌드 CSS 에 override 값·`tailwindcss-animate` 유틸·`@keyframes enter/exit` 생성 확인. 브라우저 검증(프로덕션 `next start`).

### ✅ PR 7 (quiz) — 전 도메인 사용부 전환 완료
- **사용부:** quiz(`game/quiz`) 15파일 전환(병렬 에이전트 2개[top+create / solve]). **신규 컴포넌트 없음** — `Button`=ui-tailwind, `Dialog` 3곳 panda 유지(ADR-012). `customScrollStyle` 공존. ProgressBar 동적 width=inline style, `pulse 1s`/`dropShadow`/`word-break:keep-all`=arbitrary 로 1:1.
- **no-op 보존:** `SelectCategorySection` 의 `textStyle:'body18.*'` 는 panda 에 없는 토큰(무효) → `body18-*` 로 두되 동일 no-op(원본 동작 보존). ⚠️ 디자인 의도가 glyph18 이었을 가능성(별도 확인 TODO).
- **검증:** type-check·`build:web` 통과, 잔존 0, 빌드 CSS 에서 `!glyph24-bold`(important)·`[animation:pulse_1s_infinite]`·`border-[3px]`·`bg-red-500` 생성 확인. 브라우저 검증(프로덕션 `next start`).

### 🎉 전 도메인 사용부 전환 완료 (auth/GNB/landing/shop/mypage/guild/quiz)
ui-tailwind 컴포넌트 인벤토리: Button/AnchorButton, TextField, TextArea, SearchBar, Skeleton, Banner(+Skeleton/List/LevelBanner/PetSelectMedium), GameCard, Checkbox, Label, ScrollArea, Tooltip, DropdownMenu, CombineChip(select). 미포팅(panda 공존): **Dialog/CommonDialog** 계열(DialogV2 PR 대상), **SplitText**(애니메이션).

### 🎉🎉 PandaCSS 완전 제거 완료 (PR8~final, 2026-06-21)
계획 외 영역(event/laboratory/dev/공유컴포넌트)까지 전부 전환 + Dialog 계열(DialogV2)·Select·Accordion·NoRatingCard·SplitText 이식 + 공유 panda 스타일(scrollStyle/prevTextToken) 전환 + **PR final(ADR-016)**: panda 인프라 제거 + tailwind preflight 재활성화. **apps/web 전체 `_panda`/`styled-system`/`@gitanimals/ui-panda` 잔존 0**, build 통과, 빌드 CSS panda 잔재 0 + preflight 적용 확인.

### 다음 액션 (마이그레이션 종료 후)
1. ⚠️ **앱 전반 브라우저 시각 검증**(preflight 가 panda reset 대체 → 타이포/여백 미세 변화 점검). — 진행 중
2. **push/머지 전략(미해결, 사용자 결정):** main 기준 39커밋을 cherry-pick/rebase 로 설계 PR들(ui-tailwind 컴포넌트/인프라) + 사용부 통합 PR + PR final 로 분리해 머지.
3. **정리(선택):** no-op 보존분(panda 미정의 토큰 body18.*/brand.sky_25/gray_250)·미사용 변수 — 디자인 의도 확인 후. `@gitanimals/ui-panda` 패키지 자체는 모노레포 잔존(타 앱 사용 가능).
4. **PR final:** 1~2 완료 후 PandaCSS 완전 제거(`styled-system`/`@gitanimals/ui-panda`/panda config) + tailwind preflight 재활성화(ADR-002) + 잔존 `grep -rE "_panda|styled-system|ui-panda"` 0 확인.
