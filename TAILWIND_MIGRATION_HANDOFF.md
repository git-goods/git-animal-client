...
### 다음 액션 (사용부 전환 종료 후 남은 작업)
1. **Dialog→DialogV2 전역 별도 PR**(ADR-012): shop/mypage/guild/quiz 등 panda `Dialog`/`CommonDialog`/`dialogTitleStyle` 사용처 일괄 마이그레이션. ui-tailwind 에 Dialog 계열 추가(애니메이션은 표준 shadcn — ADR-015 방식).
2. **`TopBanner` 의 ui-panda `SplitText` → ui-tailwind 이식**(ADR-010).
3. **push 전략(미해결):** 전체 포팅 후 cherry-pick/rebase 로 설계 PR들(컴포넌트 추가/border-solid/embla 등) + 사용부 통합 PR 로 분리해 merge(사용자 결정).
