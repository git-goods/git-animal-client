...
### 다음 액션
1. **push 전략(미해결):** 전체 포팅을 다 진행한 뒤 cherry-pick/rebase 로 커밋을 반영해 PR merge 예정(사용자 결정). 브랜치를 PR0→PR1→PR2→… 로 쌓는다.
2. **§4 매핑표:** DECISIONS 의 실제 매핑과 정합. 슬라이스마다 등장 토큰 검수.
3. **PR3 슬라이스:** 권장 순서 `landing → shop → mypage → guild → quiz`(§7). landing 은 `Footer.style.ts`(별도 .style.ts → §5 turbopack 함정) 포함하니 인라인/코로케이션으로 전환. 컴포넌트는 panda 원본 1:1 대조 후 ui-tailwind 추가. flicking→embla, Dialog→DialogV2 동반.
