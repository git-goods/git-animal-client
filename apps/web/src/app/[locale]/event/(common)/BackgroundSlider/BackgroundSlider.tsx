...
function BackgroundSlider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-[60px] relative mobile:px-[35px]">
      <div className="relative w-full mt-[20px] mobile:mt-0">
        ...
        <Flicking ref={flicking} {...sliderOptions}>
          {Children.map(children, (child, idx) => (
            <div
              className="w-[calc(50%_-_10px)] p-[10px] text-center [&_img]:w-full [&_img]:h-auto [&_img]:rounded-[8px] [&_p]:mt-[10px] [&_p]:text-[16px]"
              key={idx}
            >
              {child}
            </div>
