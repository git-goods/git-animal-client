  ...
  return (
    <>
      <Button
        ...
        className="w-[230px] h-[76px] mt-[63px] mx-auto glyph28-bold"
        onClick={onClickDraw}
      >
        {drawButtonText}
      </Button>
      ...
      <AnimatePresence mode="wait">
        {drawedPet && (
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-drawer" ref={modalRef}>
            <motion.div
              className="p-[24px] bg-gray-150 rounded-[16px] text-white min-w-[400px] h-auto flex flex-col justify-center items-center"
              variants={modalVariants}
              initial="initial"
              animate="animate"
              exit="exit"
            >
              <span className="glyph28-bold mb-[20px]">{t('result-title')}</span>
              {renderCard(drawedPet)}

              <Link href="/mypage" className="glyph16-bold mt-[24px] mb-[12px] cursor-pointer underline">
                {t('result-apply')}
              </Link>
