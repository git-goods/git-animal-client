...
const QuizProgressBar = ({ onTimeout, timeoutAt, paused }: QuizProgressBarProps) => {
  ...
  return (
    <div className="h-1 w-full overflow-hidden rounded bg-white/25">
      <div
        className={cn(
          'h-full rounded transition-[width,background-color] duration-100 ease-linear',
          warning ? 'animate-pulse bg-red-500' : 'bg-white',
        )}
