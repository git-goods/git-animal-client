...
- 2026-06-22 (flicking 완전 제거): 마지막 flicking 사용처 event `BackgroundSlider` 를 embla 로 전환(panelsPerView→slidesToScroll, 화살표 disabled→canScrollPrev/Next). CardList/layout/page 의 flicking.css import 제거, `@egjs/react-flicking`·`@egjs/flicking-plugins` deps 제거. → **flicking→embla 완료**(landing/guild/shop/event 전부 embla). 세 목표(panda→tailwind / flicking→embla / dialog→ui-tailwind V2) 모두 완료.
